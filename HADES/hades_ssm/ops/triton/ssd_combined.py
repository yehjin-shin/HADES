...
def _chunk_scan_chunk_state_bwd_dx_kernel(
    ...
    if IS_TRITON_22 and BLOCK_SIZE_DSTATE <= 128:
        ...
        acc *= scale[:, None] # scale : a_k <= final acc = a_k (h_k+1) 
...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
        ...
        # import pdb; pdb.set_trace()
        assert activation in [None, "silu", "swish"], "Error point 1"
        if D.dim() == 1:
            assert headdim is not None
            nheads, = D.shape
        else:
        ...
        dim = nheads * headdim
        assert nheads % ngroups == 0, "Error point 2"

        dstate = (conv1d_weight.shape[0] - dim) // ngroups // 2
        d_nonssm = (zxbcdt.shape[-1] - 2 * dim - 2 * ngroups * dstate - nheads) // 2

        assert d_nonssm >= 0, "Error point 3"
        assert zxbcdt.shape == (batch, seqlen, 2 * d_nonssm + 2 * dim + 2 * ngroups * dstate + nheads), "Error point 4"
        # assert dt_bias.shape == (nheads,), "Error point 5"
        assert A.shape == (nheads,), "Error point 6"
