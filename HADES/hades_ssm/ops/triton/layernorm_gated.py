...
class RMSNorm(torch.nn.Module):
    ...
    def __init__(self, hidden_size, eps=1e-5, group_size=None, norm_before_gate=True, device=None, dtype=None):
        ...
        self.register_parameter("bias", None)
        self.reset_parameters()
        # self.weight.register_hook(self._gradient_hook)

    def reset_parameters(self):
        torch.nn.init.ones_(self.weight)

    # def _gradient_hook(self, grad):
    #     """Gradient를 같은 위치 그룹끼리 평균내어 업데이트"""
    #     hidden_size = self.hidden_size
    #     group_size = self.group_size
    #     num_groups = hidden_size // group_size  # 총 그룹 개수

    #     # (batch, num_groups, group_size) 형태로 변환하여 위치별 평균 계산
    #     grad_view = grad.view(num_groups, group_size)  # (num_groups, group_size)
    #     grad_mean = grad_view.mean(dim=0, keepdim=True)  # (1, group_size) → 같은 위치끼리 평균

    #     # 같은 위치의 weight는 동일한 gradient 적용
    #     return grad_mean.expand_as(grad_view).reshape_as(grad)
