...
def _bmm_chunk_fwd(a, b, chunk_size, seq_idx=None, causal=False, output_dtype=None): # 나눔
    """
    Argument:
    ...
    out = torch.empty((batch, nchunks, chunk_size, chunk_size) if not has_groups else (batch, nchunks, ngroups, chunk_size, chunk_size),
                      device=a.device, dtype=out_dtype) # out 저장될 텐서 만들기
    ...
    grid = lambda META: (triton.cdiv(chunk_size, META['BLOCK_SIZE_M']) * triton.cdiv(chunk_size, META['BLOCK_SIZE_N']),
                    batch, nchunks if not has_groups else nchunks * ngroups) # 3차원 grid 설정
    with torch.cuda.device(a.device.index):
        _bmm_chunk_fwd_kernel[grid]( # 나눠서 들어가는 현장
            a, b, out, seq_idx,
