    ...
    **kwargs
    ...
    def get_logits(input_ids, inference_params):
        ...
        if not cg or not decoding:
            logits_and_hiddenstates = model(
                input_ids,
                position_ids=position_ids,
                inference_params=inference_params,
                num_last_tokens=1,
            )
            logits = logits_and_hiddenstates.logits.squeeze(dim=1)
            hidden_states = logits_and_hiddenstates.hidden_states
        else:
    ...
    def generate(
        ...
        **kwargs,
        ...
        if input_ids.shape[1] > 2048:
            teacher_outputs = input_ids[:]
            input_ids = input_ids[:, :2048]
        else:
            teacher_outputs = None

        output = decode(
            input_ids, self, max_length, top_k=top_k, top_p=top_p, min_p = min_p, temperature=temperature, teacher_outputs=teacher_outputs, **kwargs # 
        )
